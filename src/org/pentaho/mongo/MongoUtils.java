...
/**
 * Static utility routines for MongoDB. Initialize and configure a connection, retrieve replica set member info from the
 * system.replset collection, retrieve tag sets defined for replica set members, get any custom getLastErrorModes etc.
 * 
...
public class MongoUtils {
   ...
   *          the username to use
   * @param password
   *          the password to use (normal authentication only)
   * @param dbName
   *          the database to authenticate against (normal authentication only)
   * @param kerberos
   *          true if Kerberos authentication is to be used
   ...
   * 
   * @param meta
   *          MongoDBOutputMeta to use for getting username, password etc.
   * @param vars
  ...
  public static MongoCredential createCredentials( MongoDbOutputMeta meta, VariableSpace vars ) {
    ...
    String realPass = Encr.decryptPasswordOptionallyEncrypted( vars.environmentSubstitute( meta.getPassword() ) );

    return createCredentials( realUser, realPass, vars.environmentSubstitute( meta.getDBName() ), meta
   ...
   * 
   * @param meta
   *          MongoDBInputMeta to use for getting username, password etc.
   * @param vars
   *          for resolving environment variables
   * @return a configured MongoCredential object
   */
  public static MongoCredential createCredentials( MongoDbInputMeta meta, VariableSpace vars ) {
    ...
    String realPass =
        Encr.decryptPasswordOptionallyEncrypted( vars.environmentSubstitute( meta.getAuthenticationPassword() ) );
  ...
  public static void configureConnectionOptions( MongoClientOptions.Builder optsBuilder, String connTimeout,
    ...
    if ( tagSet != null && tagSet.size() > 0 ) {
      ...
      if ( log != null
        ...
        log.logBasic( BaseMessages.getString( PKG, "MongoUtils.Message.UsingReadPreferenceTagSets", builder.toString() ) ); //$NON-NLS-1$
      }
    } else {
      if ( log != null ) {
    ...
    if ( Const.isEmpty( writeConcern ) && Const.isEmpty( wTimeout ) && !journaled ) {
      ...
      if ( !Const.isEmpty( writeConcern ) ) {
        ...
        try {
          ...
          if ( log != null ) {
            String lwc =
                "w = " + writeConcern + ", wTimeout = " + wt + ", journaled = "
                    + ( new Boolean( journaled ).toString() );
            log.logBasic( BaseMessages.getString( PKG, "MongoUtils.Message.ConfiguringWithWriteConcern", lwc ) );
          }
        } catch ( NumberFormatException n ) {
  ...
  public static MongoClient initConnection( String hostsPorts, String singlePort, MongoCredential cred,
      boolean useAllReplicaSetMembers, String connTimeout, String socketTimeout, String readPreference,
      String writeConcern, String wTimeout, boolean journaled, List<String> tagSet, VariableSpace vars,
    ...
    if ( Const.isEmpty( hostsPorts ) ) {
      throw new KettleException( BaseMessages.getString( PKG, "MongoUtils.Message.Error.EmptyHostsString" ) ); //$NON-NLS-1$
    }
    ...
    for ( String part : parts ) {
      ...
      if ( hp.length > 2 ) {
        throw new KettleException( BaseMessages.getString( PKG, "MongoUtils.Message.Error.MalformedHost", part ) ); //$NON-NLS-1$
      }
    ...
    try {
      ...
      if ( cred == null ) {
        return ( repSet.size() > 1 || ( useAllReplicaSetMembers && repSet.size() >= 1 )
            ? new MongoClient( repSet, opts ) : ( repSet.size() == 1 ? new MongoClient( repSet.get( 0 ), opts )
   ...
   *           if a problem occurs
   */
  public static MongoClient initConnection( MongoDbOutputMeta meta, VariableSpace vars, MongoCredential cred,
      LogChannelInterface log ) throws KettleException {
    ...
    return initConnection( hostsPorts, singlePort, cred, useAllReplicaSetMembers, connTimeout, sockTimeout,
   ...
   *           if a problem occurs
   */
  public static MongoClient initConnection( MongoDbInputMeta meta, VariableSpace vars, MongoCredential cred,
      LogChannelInterface log ) throws KettleException {
    String hostsPorts = meta.getHostnames();
    String singlePort = meta.getPort();
   ...
   *          the hosts to use
   * @param singlePort
   *          the default port to use if no ports are given in the hostsPorts spec
   * @param username
  ...
  public static List<String> getLastErrorModes( String hostsPorts, String singlePort, MongoCredential cred,
    ...
    try {
      ...
      mongo =
          initConnection( hostsPorts, singlePort, cred, false, null, null, null, null, null, false, null, vars, log );
  ...
  public static List<String> getLastErrorModes( MongoDbOutputMeta meta, VariableSpace vars, MongoCredential cred,
    ...
    return getLastErrorModes( meta.getHostnames(), meta.getPort(), cred, vars, log );
  }
   ...
   * 
   * @param meta
   *          the MongoDbInput containing settings to use
   * @param vars
   ...
   *           if a problem occurs
   */
  public static List<String> getAllTags( MongoDbInputMeta meta, VariableSpace vars, MongoCredential cred,
      LogChannelInterface log ) throws KettleException {
   ...
   *          the hosts to use
   * @param singlePort
   *          the default port to use if no ports specified in hostsPorts spec
   * @param username
   ...
   *           if an error occurs
   */
  public static List<String> getAllTags( String hostsPorts, String singlePort, MongoCredential cred,
      VariableSpace vars, LogChannelInterface log ) throws KettleException {
  ...
  public static List<DBObject> getReplicaSetMembersThatSatisfyTagSets( List<DBObject> tagSets, MongoDbInputMeta meta,
    ...
    return getReplicaSetMembersThatSatisfyTagSets( tagSets, meta.getHostnames(), meta.getPort(), cred, vars, log );
  }
   ...
   *           if a problem occurs
   */
  public static List<DBObject> getReplicaSetMembersThatSatisfyTagSets( List<DBObject> tagSets, String hostsPorts,
      String singlePort, MongoCredential cred, VariableSpace vars, LogChannelInterface log ) throws KettleException {
  ...
  protected static void checkForReplicaSetMembersThatSatisfyTagSets( List<DBObject> tagSets, List<DBObject> satisfy,
      BasicDBList members ) {
    if ( members != null && members.size() > 0 ) {
      for ( int i = 0; i < members.size(); i++ ) {
        Object m = members.get( i );
    ...
    try {
      ...
      if ( local != null ) {
        ...
        if ( replset != null ) {
          ...
          if ( config != null ) {
            ...
            if ( members instanceof BasicDBList ) {
              if ( ( (BasicDBList) members ).size() == 0 ) {
                ...
                if ( log != null ) {
                  log.logBasic( BaseMessages.getString( PKG, "MongoUtils.Message.Warning.NoReplicaSetMembersDefined" ) ); //$NON-NLS-1$
                }
              } else {
          ...
          if ( log != null ) {
            log.logBasic( BaseMessages.getString( PKG, "MongoUtils.Message.Warning.ReplicaSetCollectionUnavailable" ) ); //$NON-NLS-1$
          }
        }
      } else {
  ...
  /**
   * Connect to mongo and retrieve any replica set members defined in the local.system.replset collection. Note that
   * this method is not actually needed for configuring a connection to a replica set or sharded mongo cluster (mongos)
   * as the driver will determine this automatically.
   * 
   * @param hostsPorts
   *          the host(s) and port(s) to use for initiating the connection
   * @param singlePort
   *          default port to use if none specified in the hostsPorts string
   * @param username
   ...
   *           if a problem occurs
   */
  public static List<ServerAddress> getReplicaSetMembers( String hostsPorts, String singlePort, MongoCredential cred,
      VariableSpace vars, LogChannelInterface log ) throws KettleException {
   ...
   * @return
   * @throws KettleException
   *           Error logging in as the provided user.
   */
  public static AuthContext createAuthContext( MongoDbInputMeta meta, VariableSpace varSpace ) throws KettleException {
    return createAuthContext( meta.getUseKerberosAuthentication(), varSpace.environmentSubstitute( meta
  ...
  public static Set<String> getCollectionsNames( final MongoDbInputMeta meta, final TransMeta varSpace,
    ...
    try {
      ...
      if ( ex.getCause() instanceof KettleException ) {
        ...
        throw new KettleException( "Unable to retrieve collection names for database " + dB + " from MongoDB", ex
            .getCause() );
      }
    }
  }
  ...
  public static void main( String[] args ) {
    try {
      ...
      List<String> repSetTags = MongoUtils.getAllTags( hostPort, defaultPort, null, vars, null );
      ...
      List<ServerAddress> repSetMembers = MongoUtils.getReplicaSetMembers( hostPort, defaultPort, null, vars, null );
