          ...
          if ( !isStopped() ) {
            ...
            if ( m_meta.getUpsert() ) {
              DBObject updateQuery =
                  m_data.getQueryObject( m_data.m_userFields, getInputRowMeta(), row, MongoDbOutput.this,
                      m_mongoTopLevelStructure );
  ...
  protected void commitUpsert( DBObject updateQuery, DBObject insertUpdate ) throws KettleException {
    ...
    while ( retrys <= m_writeRetries && !isStopped() ) {
      ...
      try {
        ...
        // doesnt
        result = m_data.getCollection().update( updateQuery, insertUpdate, true, m_meta.getMulti() );
